    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: 'var(--text-primary)' }}>Settings</h2>
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <Section title="Search Filters">
            ...
            <input
              ...
              placeholder="python developer, fastapi, automation"
              style={inputStyle}
            />
            ...
            <input
              type="number"
              value={form.max_jobs_per_run || '10'}
              onChange={(e) => set('max_jobs_per_run', e.target.value)}
              min={1}
              max={50}
              style={inputStyle}
            />
          ...
          <Section title="Freelancer Profile">
            ...
            <input
              ...
              placeholder="Python, FastAPI, React, AI/ML"
              style={inputStyle}
            />
            <Label>Bio / Summary</Label>
            <textarea
              value={form.freelancer_bio || ''}
              onChange={(e) => set('freelancer_bio', e.target.value)}
              placeholder="Brief professional bio for cover letter generation..."
              rows={4}
          ...
          <Section title="Attachments">
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <input type="file" accept=".pdf" onChange={handleResumeUpload} style={{ fontSize: 13 }} />
              {form.resume_path && (
                <span style={{ fontSize: 11, color: '#22c55e' }}>✓ Uploaded</span>
              )}
            </div>
            <Label>Portfolio PDF</Label>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <input type="file" accept=".pdf" onChange={handlePortfolioUpload} style={{ fontSize: 13 }} />
              {form.portfolio_path && (
                <span style={{ fontSize: 11, color: '#22c55e' }}>✓ Uploaded</span>
              )}
            </div>
          ...
          <Section title="Browser">
            <Label>Chrome Profile Path (for Upwork login cookies)</Label>
            <input
              value={form.chrome_profile || ''}
              onChange={(e) => set('chrome_profile', e.target.value)}
              placeholder="/Users/you/Library/Application Support/Google/Chrome/Default"
              style={inputStyle}
            />
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
              Leave empty to use a fresh browser session (you'll need to log in manually).
            </div>
          </Section>
